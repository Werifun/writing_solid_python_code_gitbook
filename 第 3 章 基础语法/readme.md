...
* C++中，由于定义(defination)和声明(declaration)的存在，变量b在定义的时候，已经分配好了__内存__，改变b的值就是改变变量b中对应内存中的值。
* Python中，从文中例子来看(P75-76)，变量b类似于c++中的__指针__，指向一个内存(的地址)，该内存中通常存储着数据，通过读取指向的内存获得该变量的值，通过改变指向的内存地址实现值的修改。
